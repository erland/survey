...
@QuarkusTest
class SurveyApiTest {
    @Inject SurveyRunRepository surveyRunRepository;
    ...
    @Test
    void deletingSurveyAlsoDeletesRunsAndTheirPublicAccess() {
        String cookie = login();
        String accountId = accountId(cookie);
        String surveyId = given().cookie("survey_admin_session", cookie).contentType(ContentType.JSON)
                .body("""
                    {"title":"Raderas helt","status":"DRAFT","questions":[
        ...
        given().post("/api/public/runs/{publicId}/participants", publicId)
                .then().statusCode(201);

        given().cookie("survey_admin_session", cookie)
                .delete("/api/admin/accounts/{accountId}/surveys/{surveyId}", accountId, surveyId)
                .then().statusCode(204);

        org.junit.jupiter.api.Assertions.assertNull(
                surveyRunRepository.findById(java.util.UUID.fromString(runId)));
